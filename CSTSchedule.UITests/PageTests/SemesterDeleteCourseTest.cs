...
{
    class SemesterDeleteCourseTest
    {
        EdgeDriver driver;
        ...
        [Test]
        public void Test_CancelModal()
        {
            string courseDeleteBtn = "#CourseTable > tr:nth-child(11) > td:nth-child(11) > button";
            string courseCancelDeleteButton = "#divButtons > button:nth-child(2)";
            string datePointList = "body > div.page > div.main > div.content.px-4 > div:nth-child(3) > ul";
            string thirdDatePoint = "body > div.page > div.main > div.content.px-4 > div:nth-child(3) > ul > li:nth-child(3) > button";
            string wednesday8CourseSelector = "body > div.page > div.main > div.content.px-4 > div:nth-child(3) > div > div > div > table > tbody > tr:nth-child(1) > td:nth-child(4) > div > div > form > div:nth-child(1) > input";
            ...
            Assert.IsTrue(getWebElement(courseCancelDeleteButton).Displayed);

            // click cancel button to cancel deletion
            getWebElement(courseCancelDeleteButton).Click();
            Thread.Sleep(TEST_WAIT);

            #region loop through table to check for course startLateCourse
            //assert that the course still exists
            ...
            //check that the third date point is still there
            IWebElement ul = getWebElement(datePointList);
            var liList = ul.FindElements(By.TagName("li"));
            Assert.IsTrue(liList.Count == 3);

            //click on third datepoint
            getWebElement(thirdDatePoint).Click();
            Thread.Sleep(TEST_WAIT);

            //check that the cisr object at wed8 is still there too
            var course = getWebElement(wednesday8CourseSelector);
            Assert.IsTrue(course.GetAttribute("value") == "[startLateCourse] startLateCourse");
